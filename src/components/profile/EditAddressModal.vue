...
export default {
  ...
  setup(props, { emit }) {
    ...
    const villages = ref([])
    const form = ref({
      province_id: props.user.addresses[props.index].province_id || '',
      city_id: props.user.addresses[props.index].city_id || '',
      district_id: props.user.addresses[props.index].district_id || '',
      village_id: props.user.addresses[props.index].village_id || '',
    ...
    fetchProvinces()
    if (form.value.province_id) fetchRegencies(form.value.province_id)
    if (form.value.city_id) fetchDistricts(form.value.city_id)
    if (form.value.district_id) fetchVillages(form.value.district_id)
    ...
    const submit = () => {
      ...
      emit('update-address', props.index, { ...form.value })
    }
